...
const [gptResponse, setgptResponse] = useState('');
  ...
  const gptSubmit = async (e) => {
    e.preventDefault();

    //入力されたデータをコンソールに表示
    console.log("入力情報:", inputgpt);

    const res = await fetch(`http://127.0.0.1:5000/api/gpt/${inputgpt}`, {
      method: 'GET',
    });
    const data = await res.json();
  ...
  return (
    <div>
      ...
      {gptResponse && <p>GPTからの応答: {gptResponse}</p>}
